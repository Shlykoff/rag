...
//
// Pure unit tests for lib/ai/crypto.ts's AES-256-GCM encrypt/decrypt round
// trip -- no I/O, no Supabase needed. Mirrors lib/sources/__tests__/crypto.test.ts
// exactly (same scheme, same env var, same failure modes) since lib/ai/crypto.ts
// is a deliberate near-duplicate of that module -- see its own header for why
// it isn't just imported instead. Sets CREDENTIALS_ENCRYPTION_KEY itself (via
// vi.stubEnv) rather than relying on .env.local, so this test is
// self-contained and doesn't silently pass/fail based on local dev env setup.
// No module-reset gymnastics needed: crypto.ts's loadKey() reads
...
describe("lib/ai/crypto: encrypt/decrypt round trip", () => {
  beforeEach(() => {
  ...
  it("decrypts back to the original plaintext (an OpenAI-key-shaped string)", () => {
    const plaintext = "sk-proj-abcdEFGH12345";
    const encrypted = encryptCredential(plaintext);
    expect(decryptCredential(encrypted)).toBe(plaintext);
  });

  it("decrypts back a Voyage-key-shaped string with unicode/special characters thrown in for good measure", () => {
    const plaintext = "pa-ZZZ_fake-key-тест-🔑-1234567890";
    const encrypted = encryptCredential(plaintext);
    expect(decryptCredential(encrypted)).toBe(plaintext);
  });
