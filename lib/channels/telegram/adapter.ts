...
// actually calls (Next.js routing is already channel-specific per the URL
// path, so the route doesn't need to go through lib/channels/index.ts's
// generic-dispatch-by-name registry; that registry exists for future
// callers that DO need to look an adapter up generically by a channel
// string, e.g. an admin tool).
//
