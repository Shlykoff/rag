...
- [ ] **Frontend** (`nextjs-frontend`): chat UI, source-adding screens, history. Not built yet — both `app/api/chat/route.ts` and `app/api/sources/*` are server-only API routes with documented contracts (see below) for the frontend to consume.
...
```bash
npm test                  # fast unit tests, no Docker required (153 tests)
...
- ✅ **Verified with fast unit tests against fakes** (`npm test`, 153 tests): chunking edge cases, the AI-provider error normalizer, the retry/backoff logic (including the retry-before-first-token bug the tests caught), embedding batch bisection, the rate limiter's query logic, the retrieval context-assembly/token-budget logic, the full chat request pipeline (conversation creation/reuse, message persistence, provider-failure handling), and (from `document-sources-specialist`) the SSRF guard (including the IPv6-bracket classification fix), credential encryption, and the source-ingest rate limiter.
...
**No real Notion/Google credentials are available in this environment** (same situation as the AI provider keys above — the user will add them later). What's actually been exercised:
...
- ⚠️ **Not yet verified**: an actual Notion API call (`@notionhq/client` request/response shapes accepted by the real API, the page/database-id-guessing fallback in `importNotionDocument`, the "not shared" 404 translation) or an actual Google Drive API call (`googleapis` auth against a real service account, folder listing, Google Docs export, the "not shared" 403/404 translation). Both adapters are written and type-checked against each SDK's real, installed TypeScript types (not guessed), and their request/response shapes match the SDKs' documented contracts, but neither has round-tripped against the live Notion/Google APIs. **First thing to check once credentials are available**: create a Notion internal integration + share a test page with it, and/or create a Google Cloud service account + share a test Drive folder with its `client_email` — see "How to test each source manually" below.
