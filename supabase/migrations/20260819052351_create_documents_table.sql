...
create table public.documents (
  ...
  -- notion/url/google_drive. Always prefixed with `${user_id}/...` so the
  -- Storage RLS policies can enforce ownership from the path alone.
  ...
  -- DB-level enforcement of the "<user_id>/..." Storage path convention
  -- described above (previously only a comment, not actually checked).
  -- Allows NULL (see storage_path comment: the row may exist before the
  -- object is uploaded); only validates the prefix once a path is set, so
  -- it can never diverge from what the storage.objects RLS policies
  -- (documents_bucket_*_own, see the storage bucket migration) expect --
  -- both read the same "${user_id}/..." convention off the same user_id.
  constraint documents_storage_path_prefixed_with_user_id check (
...
comment on table public.documents is
  'One row per ingested document (any source: manual upload, Notion, public URL, Google Drive). Original/cached text lives in the private "documents" Storage bucket at storage_path.';
...
comment on column public.documents.storage_path is
  'Object path in the "documents" Storage bucket. Nullable until the object is actually uploaded (e.g. processing_status = pending); when set, must start with "<user_id>/" (enforced by documents_storage_path_prefixed_with_user_id) so storage.objects RLS policies can enforce per-owner access from the path.';
...
-- Shared updated_at trigger function, defined once here and reused by
-- every other table below that has an updated_at column (avoids taking a
-- dependency on the optional moddatetime extension).
...
-- explicit policy. Only the owner (auth.uid() = user_id) can see/change
-- their own documents. The server also writes to this table using the
